...
class EnhancedWebGUI:
    ...
    def setup_routes(self):
        ...
        def api_get_profiles():
            ...
            try:
                profiles = self.get_all_profiles()
                return jsonify({'success': True, 'profiles': list(profiles.keys())})
            except Exception as e:
                self.error_handler.log_error('profiles_error', f'Failed to get profiles: {e}')
                return jsonify({'success': False, 'error': str(e)})

        @self.app.route('/api/proxies')
        def api_get_proxies():
            """Get all verified proxies"""
            try:
                return jsonify({'success': True, 'proxies': self.verified_proxies})
            except Exception as e:
                self.error_handler.log_error('proxies_error', f'Failed to get proxies: {e}')
                return jsonify({'success': False, 'error': str(e)})
            ...
            try {
                ...
                if (data.success) {
                    ...
                    data.profiles.forEach(profile => {
                        ...
                        option.textContent = profile.replace(/_/g, ' ').replace(' profile', '').toUpperCase();
        ...
        function loadProxyData() {
            fetch('/api/proxies').then(res => res.json()).then(data => {
                ...
                if (data.success && data.proxies.length > 0) {
                    ...
                    thead.innerHTML = '<tr><th>Proxy</th><th>Country</th><th>City</th><th>RTT</th><th>Status</th></tr>';
                    const tbody = document.createElement('tbody');
                    data.proxies.forEach(p => {
                        const tr = document.createElement('tr');
                        tr.innerHTML = `<td>${p.proxy}</td><td>${p.country || 'N/A'}</td><td>${p.city || 'N/A'}</td><td>${p.rtt_ms}ms</td><td>Working</td>`;
                        tbody.appendChild(tr);
                    });
                    table.appendChild(thead);
                    table.appendChild(tbody);
                    proxyResults.appendChild(table);
                } else {
                    proxyResults.innerHTML = '<div class="alert alert-info">No verified proxies available.</div>';
                }
            });
        }
        // loadCookieProfiles is deprecated and replaced by loadProfiles
        function loadProfileData() {
            const profileName = document.getElementById('cookieProfileSelect').value;
            if (!profileName) {
                document.getElementById('profileAnalytics').innerHTML = '<div class="alert alert-info">Select a profile to view analytics.</div>';
                return;
            }

            fetch(`/api/profile/analytics/${profileName}`)
                .then(res => res.json())
                .then(data => {
                    if (data.success) {
                        ...
                        let html = `
                            <h4>Analytics for ${profile.profile_name || profile.profile_id}</h4>
                            <p><strong>Anonymity Score:</strong> ${analytics.anonymity_score}/100</p>
                            <p><strong>Profile Health:</strong> ${analytics.profile_health}</p>
                            <h5>Score Breakdown:</h5>
                            <ul>
                                <li>Profile Integrity: ${analytics.score_breakdown.profile_integrity}/100</li>
                                <li>Cookie Quality: ${analytics.score_breakdown.cookie_quality}/100</li>
                                <li>Security Measures: ${analytics.score_breakdown.security_measures}/100</li>
                                <li>Usage Patterns: ${analytics.score_breakdown.usage_patterns}/100</li>
                            </ul>
                        ...
                        `;
                        document.getElementById('profileAnalytics').innerHTML = html;
                    } else {
                        document.getElementById('profileAnalytics').innerHTML = `<div class="alert alert-danger">Error loading analytics: ${data.error}</div>`;
                    }
                })
                .catch(error => {
                    console.error('Analytics load failed:', error);
                    document.getElementById('profileAnalytics').innerHTML = '<div class="alert alert-danger">Error loading analytics.</div>';
                });
        }
